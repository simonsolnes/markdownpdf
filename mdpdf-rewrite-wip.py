...
def conv_header(line):
	...
	return '\\' + subs + 'section{' + conv_span(line[level + 1:]) + '}'
...
def conv_document(path):
	...
	for line in iterlines:
		# heading
		if re.match('#+\s', line):
			tex.append(conv_header(line))
		# latex
		elif re.match('``latex$', line):
			tex += conv_latexcode(iterlines)
		# bibliography
		elif re.match('``bib$', line):
			tex.append('\\printbibliography') # TODO: this needs to be done by somebody else
			generate_references(iterlines, path[:-3] + '.bib')
			uses_bib = True
		'''
		# list
...
def generate_references(iterlines, path):
	#TODO: this function is shittily written
	block = []
	line = next(iterlines)
	while line != '``':
		block.append(line)
